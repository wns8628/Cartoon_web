...
var userSchema = mongoose.Schema({
 ...
 name:{
   ...
   match:[/^.{2,6}$/,"닉네임은 2~6글자만 사용할 수 있습니다."], // 1-2
   trim:true
  },
 email:{
   type:String,
...
// password validation //
var passwordRegex = /^(?=.*[a-z])(?=.*[0-9])(?=.*[!@#$%^*()\-_=+\\\|\[\]{};:\'",.<>\/?])*.{8,}$/i; // 8-16자리 문자열 중에 숫자랑 영문자가 반드시 하나 이상 존재해야 한다는 뜻의 regex입니다.
var passwordRegexErrorMessage = "8글자이상 알파벳,숫자(특수문자) 혼합하여 비밀번호를 생성하십시오"; // 에러메세지가 반복되므로 변수로 선언하였습니다.
