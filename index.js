...
//디비세팅
mongoose.connect(process.env.maimai);
...
//사이트명바꿀떄이 변수를써라
app.use(function(req,res,next){
  res.locals.Site_name = "MaiMai";
  res.locals.mangasave = 0;
  next();
});

//라우터
app.use("/", require("./routes/home"));
app.use("/toon", require("./routes/manhwa"));
app.use("/posts", require("./routes/posts"));
app.use("/users", require("./routes/users"));
app.use("/manga_ajax", require("./routes/manga_ajax"));
// app.use("/mini", require("./routes/manhwa_mini"));
...
// ***설명 a.tx-link 이건 메인장시시에서 완료만화리스트
//        div#manga-list a   이건 연재중 ㅇㅋ ?
// 완결은 카테고리를 zang_finish 로바꿔서해~

// require('./customLibrary/save/listSave_Zang')(); //리스트들    **처음실행은저장 두번쨰실행은 포토가져오는거 두번쨰는 만화저장후 실행ㅇㅋ ?
// require('./customLibrary/save/listSave_Mini')(); //리스트들
...
//33=소드아트
//9는 원펀맨
// var mangasave = 0; // name:"일곱개의 대죄"
// ManhwaList.find({category:"zang"},function(err,manhwaList){
//   var forstop  = setInterval(function(){
//                  require('./customLibrary/save/PostSave_Zang')(manhwaList[mangasave].url, manhwaList[mangasave].name);
//                  mangasave++;
//                  if(mangasave == manhwaList.length){
...
//*은혼은 없데이트할때마다 1000 2000 3000 지금 3천까지나옴 다음3천 이렇게 천단위로 넣어라 인덱스
// ManhwaList.find({name:"은혼"},function(err,manhwaList){
...
// require('./customLibrary/save/PostSave_Mini')('http://minitoon.net/bbs/board.php?bo_table=9998&wr_id=17563','노아즈 노츠 ');
