...
//월일만
util.getDate_md = function(dateObj){
 if(dateObj instanceof Date)
  return get2digits(dateObj.getMonth()+1)+ "/" + get2digits(dateObj.getDate());
}
...
//만화검색
util.mangaSearch = function(queries){

  var findPost = {};
  if(queries){
    // var searchTypes = queries.searchType.toLowerCase().split(",");
    var postQueries = [];
      postQueries.push({ name : { $regex : new RegExp(queries, "i") } });

      findPost = {$or:postQueries};
  }
  return { searchText:queries,
           findPost:findPost};
  }
